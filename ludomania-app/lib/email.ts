...
const fromEmail = 'Ludomania <onboarding@resend.dev>'; // Resend test domain
  ...
  try {
    await resend.emails.send({
      ...
      html: `
        ...
        <p><strong>User ID:</strong> ${userId}</p>
        <p><strong>Amount:</strong> KSh ${amount}</p>
        <p><strong>M-Pesa Details:</strong></p>
        <pre>${bankDetails}</pre>
  ...
  try {
    await resend.emails.send({
      ...
      html: `
        ...
        <p><strong>Wager Amount:</strong> KSh ${wagerAmount}</p>
        <p><strong>Winner Received:</strong> KSh ${wagerAmount * 2}</p>
        <p>Funds have been automatically transferred.</p>
      `,
    });
    console.log('✅ Game completion notification sent to admin');
  } catch (error) {
