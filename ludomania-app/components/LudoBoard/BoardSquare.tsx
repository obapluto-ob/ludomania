...
export default function BoardSquare({
  ...
  const getSquareStyle = () => {
    const baseClasses = `relative flex items-center justify-center border transition-all ${className}`;
    ...
    switch (type) {
      ...
      case 'home':
        ...
        return `${baseClasses} bg-gradient-to-br ${homeScheme.gradient} border-2 border-${color}-900 rounded-full`;
      
      case 'finish':
        if (!color) return `${baseClasses} bg-gray-200 border-gray-400`;
        const finishScheme = COLOR_SCHEMES[color];
        return `${baseClasses} bg-${color}-200 border-${color}-400`;
      
      case 'center':
        return `${baseClasses} bg-gradient-to-br from-yellow-400 via-amber-500 to-yellow-600 border-4 border-amber-900 rounded-lg shadow-lg`;
